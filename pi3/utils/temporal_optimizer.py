...
def ensure_base_strategy_exists(
    ...
    base_dir = os.path.join(
        "aligned_outputs", model_name, dataset_type, base_strategy,
    ...
    baseline_dir = os.path.join(
        "aligned_outputs", model_name, dataset_type, "baseline",
        subject_name, f"{n_views}views",
    )
    if not os.path.isdir(baseline_dir):
        # Legacy fallback (flat hierarchy from older scripts)
        baseline_dir = os.path.join(
            "aligned_outputs",
            subject_name, f"{n_views}views",
        )
