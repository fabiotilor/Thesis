...
from mast3r.demo_video import get_args_parser, main_demo
...
if __name__ == '__main__':
    ...
    with get_context(args.tmp_dir) as tmpdirname:
        ...
        # ---------------------------------------------------------------------------
        # Hardcoded dataset: DEX-YCB multi-view sequence.
        # The path is pre-filled in the UI textbox; images are loaded directly from
        # disk at run-time (view_XX/rgb/*.png) — no Gradio file copying.
        # ---------------------------------------------------------------------------
        ...
        main_demo(cache_path, model, args.retrieval_model, args.device, args.image_size, server_name, args.server_port,
                  ...
                  default_dataset_path=DATASET_ROOT)
