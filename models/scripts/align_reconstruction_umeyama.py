...
# Umeyama alignment
from utils.umeyama_alignment import (
...
def run_reconstruction(
    ...
    for t in range(n_frames):
        ...
        try:
            ...
            in_npz_path = os.path.join("ggpt_inputs", model_name, subject_full, f"{nviews}views", f"frame_{t:05d}.npz")
            if not os.path.exists(in_npz_path):
                # Fallback to old naming frame_00.npz
                in_npz_path = os.path.join("ggpt_inputs", model_name, subject_full, f"{nviews}views",
                                           f"frame_{t:02d}.npz")
            if not os.path.exists(in_npz_path):
                print(f"[WARN] Input NPZ not found: {in_npz_path}, skipping frame.")
                continue

            data = dict(np.load(in_npz_path))

            # Extract expected tensors
            # pointmaps: (V, H, W, 3), pointmaps_confs: (V, H, W)
            pts3d_all = data['pointmaps']
            confs_all = data['pointmaps_confs']

            # Poses and intrinsics (estimated)
            est_c2w = data['est_poses']
            extrinsics_w2c = closed_form_inverse_se3(torch.from_numpy(est_c2w)).numpy()
            intrinsics_est = data['est_intrinsics']
            ...
            if use_gt_intrinsics:
                ...
                for i in range(V_inj):
                    ...
                    print(
                        f"      Focal ratio: fx_gt/fx_est={f_gt_x / f_est_x:.4f}  fy_gt/fy_est={f_gt_y / f_est_y:.4f}")
