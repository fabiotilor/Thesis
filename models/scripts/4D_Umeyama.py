...
from utils.gt import load_gt_params, build_gt_validity_masks
        ...
        try:
            ...
            if n_pts == 0:
                print(
                    ...
                    f"(Conf > Top {int(CONF_PERCENTILE*100)}% + GT Masks)."
                )
...
def evaluate_subject(subject_name, model_name="vggt-point", selected_views=None, pgo_only=False):
    base_dir = os.path.join("aligned_outputs", model_name, "baseline", subject_full) if "subject_full" in locals() else os.path.join("aligned_outputs", model_name, "baseline", subject_name)
    if not os.path.exists(base_dir):
        # try old format if new not found
    ...
    for vdir in view_dirs:
        ...
        if not pgo_only:
            ...
            log_aligned_sequence(paths, tf_s1, s_g1, R_g1, tr_g1, "Strategy_1", [0, 0, 255], dataset_root,
                                 log_root=log_root)
            save_timing(
                os.path.join("aligned_outputs", model_name, "strategy1", subject_name, vdir),
                strat1_label,
                len(paths),
            ...
            log_aligned_sequence(paths, tf_s2, s_g2, R_g2, tr_g2, "Strategy_2", [255, 0, 0], dataset_root,
                                 log_root=log_root)
            save_timing(
                os.path.join("aligned_outputs", model_name, "strategy2", subject_name, vdir),
                strat2_label,
                len(paths),
        ...
        log_aligned_sequence(paths, tf_s3, s_g3, R_g3, tr_g3, "Strategy_3", [0, 255, 0], dataset_root,
                             log_root=log_root)
        save_timing(
            os.path.join("aligned_outputs", model_name, "strategy3", subject_name, vdir),
            strat3_label,
            len(paths),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="vggt-point", help="Model backbone to run the evaluation pipeline on.")
    ...
    for s in selected:
        evaluate_subject(s, model_name=args.model, selected_views=args.views, pgo_only=args.pgo)