...
from utils.alignment_4D import strategy1_reference, strategy2_hierarchical, strategy3_pgo
...
def _parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="vggt-point",
        help="Model backbone to run the evaluation pipeline on."
    )
    parser.add_argument(
        "--pgo",
        action="store_true",
...
def main():
    ...
    print(f"[INFO] Running evaluation pipeline for model: {args.model}")

    cache_root = os.path.join(tempfile.gettempdir(), f"{args.model}_alignment_cache")
    ...
    for subject_full, code in zip(selected_subjects, codes):
        ...
        for nviews in view_counts:
            ...
            baseline_dir = os.path.join("aligned_outputs", args.model, f"baseline{suffix}", subject_full,
                                        f"{nviews}views")
            s1_dir = os.path.join("aligned_outputs", args.model, f"strategy1{suffix}", subject_full, f"{nviews}views")
            s2_dir = os.path.join("aligned_outputs", args.model, f"strategy2{suffix}", subject_full, f"{nviews}views")
            s3_dir = os.path.join("aligned_outputs", args.model, f"strategy3{suffix}", subject_full, f"{nviews}views")
