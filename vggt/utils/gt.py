...
def _load_hi4d_mesh_gt(dataset_root, t):
    """Load ground-truth mesh vertices from .obj files for Hi4D.

    GT meshes are at: {dataset_root}/frames/mesh-f00XXX.obj
    The frame filename uses zero-padded 5-digit frame numbers prefixed with f00.
    """
    import pickle

    # Primary: Try pickle cache in frames_vis (faster)
    pkl_path = os.path.join(dataset_root, "frames_vis", f"mesh-f{t:05d}.pkl")
    if os.path.exists(pkl_path):
        with open(pkl_path, 'rb') as f:
            mesh = pickle.load(f)
        return mesh['vertices']  # (N, 3) float32

    # Try .obj mesh in frames/
    obj_path = os.path.join(dataset_root, "frames", f"mesh-f{t:05d}.obj")
    if os.path.exists(obj_path):
        try:
            ...
            # Manual OBJ vertex parser as fallback
            vertices = []
...
def _load_hi4d_seg_mask(dataset_root, cam_id, t):
    """
    Load the combined segmentation mask for a given HI4D camera view and frame.
    Returns a boolean HxW array, or None if the mask file does not exist.

    Mask path: {dataset_root}/seg/img_seg_mask/{cam_id}/all/{frame_id}.png
    """
...
def build_static_gt_pointcloud(t, view_names, dataset_root,
                               flow_threshold=2.0, precomputed_masks=None,
    ...
    for vname in view_names:
        ...
        rgb_adj = get_rgb_path(view_dir, t + 1, dataset_type=dataset_type) or get_rgb_path(view_dir, t - 1, dataset_type=dataset_type)
...
def _get_correspondences_hi4d(t, view_names, pts3d_list, confs, dataset_root,
    ...
    with the VGGT estimated points.
    """
    # Normalise inputs
    if isinstance(pts3d_list, torch.Tensor):
        pts3d_list = pts3d_list.cpu().numpy()
    ...
    for i, vname in enumerate(view_names):
        ...
        # Final filtering: find valid pixels that have both estimation and GT projection
...
def build_gt_validity_masks(t, view_names, dataset_root, depth_max_m=1.5, target_hw=None, cache=None, dataset_type="dex-ycb"):
