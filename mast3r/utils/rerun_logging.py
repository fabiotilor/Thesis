...
def log_cameras_rerun(t, view_names, dataset_root, log_root):
    ...
    for vname in view_names:
        ...
        # Build RGB path - try rgb subdir or view dir directly
...
def log_alignment_results(t, gt_pts, aligned_pts, refined_pts=None, log_root="world"):
    """Used by 3D alignment scripts to visualize registration quality."""
    if gt_pts is not None:
        log_pointcloud(t, f"{log_root}/gt", gt_pts, color=[0, 255, 0])
    if aligned_pts is not None:
        log_pointcloud(t, f"{log_root}/estimated/aligned", aligned_pts, color=[0, 0, 255])
...
def log_gt_sequence(paths, log_root="4d_eval"):
    ...
    for p in paths:
        ...
        gt_pts = data['gt_pts']
        # Magnitude check: if > 10m, it's likely mm -> scale to m
...
def log_aligned_sequence(paths, frame_transforms, s_glob, R_glob, tr_glob, label, color, dataset_root,
    ...
    for i, p in enumerate(paths):
        ...
        # Log Cameras and Images only once per frame transition (from the first strategy)
        if i == 0:
        ...
        for v in range(V):
            ...
            if vmasks[v] is not None: mask &= vmasks[v]
