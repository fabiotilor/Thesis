#!/usr/bin/env python3
"""
Aggregate eval_summary_*.csv files (subjects 01–10) into a
single mean / mean±std table.

Handles both dex-ycb and Pi3 column naming conventions automatically.
...
Usage:
    ...
    python aggregate_evals.py --pattern "eval_summary_pi3_dex-ycb_*.csv"
...
# Pi3 uses longer names; map them to the canonical short names used throughout.
COLUMN_ALIASES = {
    "static_accuracy":      "static_acc",
    "dynamic_accuracy":     "dyn_acc",
    "static_completeness":  "static_comp",
    "dynamic_completeness": "dyn_comp",
}
...
def add_delta_consistency(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Called on the *aggregated* mean DataFrame so the difference is computed
    from averaged chamfer values, not averaged deltas.
    """
...
PRINT_ORDER = [
    ...
    "completeness", "accuracy",          # accuracy = Pi3 overall accuracy column
    "static_comp", "dyn_comp",
    "static_acc", "dyn_acc", "motion_gap",
    "overall_acc", "overall_comp",       # dex-ycb overall metrics (Hi4D split)
...
# Derived columns whose std cannot simply be aggregated
DERIVED = {"motion_gap", "delta_consistency"}
...
def aggregate(pattern: str) -> None:
    ...
    # Load, normalise column names, concatenate
    frames = []
    for path in files:
    ...
    # Drop columns that cannot / should not be aggregated
    ...
    if {"static_acc", "dyn_acc"}.issubset(mean_df.columns):
        mean_df["motion_gap"] = mean_df["static_acc"] - mean_df["dyn_acc"]
        std_df["motion_gap"]  = np.nan   # propagation not implemented — leave explicit
    ...
    no_std_cols = DERIVED | {"n_frames", "n_anchors", "align_frames"}
    ...
    for col in print_cols:
        ...
        for _, mrow in mean_df.iterrows():
            ...
            if pd.isna(mu):
                cell = "—"
            elif pd.isna(sd) or col in no_std_cols:
                cell = f"{mu:.5f}"
            else:
                cell = f"{mu:.5f} ± {sd:.5f}"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pattern",
        default="eval_summary_*.csv",
        help="Glob pattern for input CSV files (default: eval_summary_*.csv)",
    )
